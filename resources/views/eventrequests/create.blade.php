    ...
    <div class="max-w-[1400px] mx-auto px-6 py-12">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6 mb-10">
            <div>
                ...
                <p class="text-slate-500 mt-2 text-lg">
                    Vul het formulier in om een nieuwe event aanvraag toe te voegen.
                </p>
            </div>
        ...
        @if ($errors->any())
            <div class="bg-rose-50 border border-rose-200 text-rose-800 p-5 rounded-2xl mb-8">
                <p class="font-semibold mb-2">Er zijn een paar problemen met je invoer:</p>
        ...
        <div class="bg-white border border-slate-100 rounded-[2rem] overflow-hidden shadow-xl shadow-slate-100/50 p-8">
            <form method="POST" action="{{ route('eventrequests.store') }}" class="space-y-8">
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    <div class="space-y-3">
                        <label for="name" class="block text-sm font-semibold text-slate-700">Naam</label>
                        <input id="name" name="name" type="text" value="{{ old('name') }}"
                            ...
                            placeholder="Naam van de aanvraag">
                        @error('name')
                    ...
                    <div class="space-y-3">
                        <label for="venue_id" class="block text-sm font-semibold text-slate-700">Locatie</label>
                ...
                <div class="space-y-3">
                    <label for="description" class="block text-sm font-semibold text-slate-700">Omschrijving</label>
                    <textarea id="description" name="description" rows="6"
                        class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-slate-900 focus:border-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-200"
                        placeholder="Beschrijf de event aanvraag">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-rose-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="text-sm text-slate-500">
                    Aanvraag wordt aangemaakt voor de ingelogde gebruiker: {{ auth()->user()?->name ?? 'Onbekend' }}
                </div>
                @error('user_id')
                    <p class="text-rose-600 text-sm mt-1">{{ $message }}</p>
                @enderror
